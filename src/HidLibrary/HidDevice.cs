...
namespace HidLibrary
{
    public class HidDevice : IHidDevice
    {
        ...
        {
            //Console.WriteLine("ReadReport: Started");
            ReadReport(callback, 0);
        }

        public void ReadReport(ReadReportCallback callback, int timeout)
        {
            //Console.WriteLine("ReadReport: Creating readReportDelegate Instance...");
            var readReportDelegate = new ReadReportDelegate(ReadReport);
            //Console.WriteLine("ReadReport: Creating asyncState Instance...");
            var asyncState = new HidAsyncState(readReportDelegate, callback);
            //Console.WriteLine("ReadReport: Begin Invoke...");
            readReportDelegate.BeginInvoke(timeout, EndReadReport, asyncState);
            //Console.WriteLine("ReadReport: Completed");
        }

        public async Task<HidReport> ReadReportAsync()
        {
            //Console.WriteLine("ReadReportAsync: Started");
            //return await Task.Run(() => ReadReport());
            var readReportDelegate = new ReadReportDelegate(ReadReport);
            return await Task<HidReport>.Factory.StartNew(() => readReportDelegate.Invoke(0));
            //return await Task<HidReport>.Factory.FromAsync(readReportDelegate.BeginInvoke, readReportDelegate.EndInvoke, 0, null);
        }
        ...
        private bool WriteData(byte[] data, int timeout)
        {
            ...
            if (_deviceWriteMode == DeviceMode.Overlapped)
            {
                ...
                {
                    Console.WriteLine("Unknown Error Happended in WriteFile");
                    return false;
                }
